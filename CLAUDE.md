...
**In-app update** (`src/core/update_checker.py`, daemon thread on launch, offline-safe). Gated on `parm.DEV_MODE_PERSISTED` — the *saved* `[Developer] ENABLED` in `config.ini`, **not** the per-session `DEV_MODE`, which always resets to False at launch (so it could never gate a launch-time check). Two channels, chosen in `controller._fetch_update_info`:
- **GitHub Releases** (default, open network).
- **VPS SSH mirror** when `SSH_REMOTE`+`SSH_KEY_PATH` are set (the gov machine: HTTPS *out* is blocked but SSH works). It `scp`-pulls `latest.json`+`Setup-latest.exe`, **verifies the sha256**, runs the installer, and closes. The install race is closed by `AppMutex` in `installer.iss` (named after the app's single-instance mutex). The endpoint that receives each CI build is a route in the shalom.5784.link Node app — code + deploy notes in `vps/` (`vps/README.md`).
