...
- `chromedriver.exe` must be at the hardcoded path `C:\chromedriver\chromedriver.exe` and match the installed Chrome version. It is launched as a subprocess on `--port=9515` (`src/automation/driver_manager.py`).
