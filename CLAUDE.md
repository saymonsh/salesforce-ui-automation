...
**Packaging** (ADR-001): a per-user Windows installer is built with `pyinstaller build.spec` → `iscc installer.iss`. Writable files (`config.ini`, `draft.json`, logs) route through `src/core/paths.py`: project root in a source run, `%APPDATA%\WelfareSFAutomation\` in a frozen build (seeded on first launch). Releases and in-app updates are now automated — see **Releases & auto-update** below. See `docs/packaging.md`.
...
**Versioning.** `src/core/version.py` (`__version__`) is the single source — bump it, commit, `git tag vX.Y.Z`, push. `build.spec` reads it (writes `build/version.iss` for Inno); it also shows in the window title.

**CI** (`.github/workflows/build-release.yml`) runs *deliberately*, not on every push: a **merged** PR → build-only smoke test (is `main` still releasable?); a **`v*` tag** → build + GitHub Release + push the installer to the VPS mirror; `workflow_dispatch` → manual. A plain push to `main`, or a push to a PR branch, does **not** build. The Release/mirror steps are gated on the tag ref, so a merge build can never publish. CI fetches+caches the Flet desktop client before building so the installer is offline-self-contained.

**In-app update** (`src/core/update_checker.py`, daemon thread on launch, offline-safe). Runs on **every frozen build** (no longer dev-mode-gated) and is **surfaced in the settings panel** ("אודות ועדכונים"), not a launch popup. The checkers return a *status dict* (`state` = `available`/`current`/`error`) so the panel distinguishes "up to date" from "couldn't check" — and the error state hints at SSH (`reason` `ssh` vs `network`/`no_channel`). Clicking "עדכן עכשיו" runs `controller.apply_update`: it closes settings and shows a non-dismissible "מתעדכן…" modal with a **live download %** in the same UI tick (no flash of bare background), then closes the app. The installer is launched **`/SILENT`** (deployment bar shows, every wizard page — incl. the shortcut question — skipped); `installer.iss` drops `skipifsilent` from `[Run]` so the app still relaunches after a silent update. A human double-clicking Setup.exe still gets the full interactive wizard. Two channels, chosen in `controller._fetch_update_info`:
- **GitHub Releases** (default, open network). Download % via `urlretrieve`'s reporthook.
- **VPS SSH mirror** when `SSH_REMOTE`+`SSH_KEY_PATH` are set (the gov machine: HTTPS *out* is blocked but SSH works). It `scp`-pulls `latest.json`+`Setup-latest.exe`, **verifies the sha256**, runs the installer, and closes. Download % comes from polling the partly-written file against the manifest's `size` field (scp emits no parseable meter when piped); an old manifest without `size` falls back to an indeterminate spinner. The install race is closed by `AppMutex` in `installer.iss` (named after the app's single-instance mutex). The endpoint that receives each CI build is a route in the shalom.5784.link Node app — code + deploy notes in `vps/` (`vps/README.md`).
...
- `version.py` — the single `__version__` source (see **Releases & auto-update**). `update_checker.py` — the launch-time update check (GitHub / VPS-SSH channels, sha256-verified). `utils.py` exports `CREATE_NO_WINDOW`.
...
- **Subprocess calls from the app must pass `creationflags=CREATE_NO_WINDOW`** (`src/core/utils.py`). The frozen build is windowed (`console=False`), so any child process (taskkill on close, `scp` for log-mirror / updates) otherwise flashes a black console window.
