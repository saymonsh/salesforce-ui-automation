...
*   **טבלת הזנה מובנית**: הנתונים מוזנים בטבלה בתוך האפליקציה. אפשר לייבא קובץ Excel קיים לתוך הטבלה או להדביק ישירות מ-Excel/Sheets (Smart Paste). הטיוטה נשמרת אוטומטית (`draft.json`) ומשוחזרת בהפעלה הבאה.
...
בכל הסוגים הקלט מגיע מטבלת ההזנה שבאפליקציה — אין מצב של "הרצת קובץ" ישירות; קובץ Excel תמיד מיובא קודם לתוך הטבלה.
...
│   │   ├── data_grid.py      # טבלת ההזנה (כולל ייבוא Excel ו-Smart Paste)
...
│       ├── excel_import.py   # ייבוא קובץ Excel לתוך הטבלה
